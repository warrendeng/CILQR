...
class iLQR:
	...
	def __init__(self, dynamics, cost, final_cost, N, state_size, action_size, max_reg=1e10, hessians=False):
		...
		self._k = [None] * self.N #list of k values that are really matrices
		self._K = [None] * self.N
	...
	def control(self, xs, us, k, K, alpha = 1.0):
		...
		for i in range(self.N):
			...
			xs_new[i + 1] = self.f(xs_new[i], us_new[i], i)
		return np.array(xs_new), np.array(us_new)
	...
	def forward_rollout(self, x0, us):
		...
		for i in range(N):
			...
			xs[i + 1] = self.f(x, u, i)
			F_x[i] = self.f_x(x, u, i)
			F_u[i] = self.f_u(x, u, i)

			L[i] = self.l(x, u, i)
			L_x[i] = self.l_x(x, u, i)
			L_u[i] = self.l_u(x, u, i)
			L_xx[i] = self.l_xx(x, u, i)
			L_ux[i] = self.l_ux(x, u, i)
			L_uu[i] = self.l_uu(x, u, i)
		...
		L[-1] = self.nl(x, N)
		L_x[-1] = self.nl_x(x, N)
		L_xx[-1] = self.nl_xx(x, N)
		#np.array(xs), np.array(F_x), np.array(F_u), np.array(L), np.array(L_x), np.array(L_u), np.array(L_xx), np.array(L_ux), np.array(L_uu), np.array(F_xx), np.array(F_ux), np.array(F_uu)
